...
const EvaluationDemo: React.FC = () => {
    ...
    useEffect(() => {
        ...
        fetch(`${process.env.PUBLIC_URL}/demo_data_ewars.json`) // Using PUBLIC_URL to ensure compatibility with different environments
            ...
            .then((data) => {
                evaluationForSplitPoint = addModelName(
                    data.predictions,
                    'EWARS model'
                )
            })
        ...
        fetch(`${process.env.PUBLIC_URL}/demo_data_ar.json`) // Using PUBLIC_URL to ensure compatibility with different environments
            .then((response) => response.json())
            .then((data) => {
                evaluationForSplitPoint = evaluationForSplitPoint.concat(
                    addModelName(data.predictions, 'AR model')
                )
                const processedData = evaluationResultToViewData(
                    evaluationForSplitPoint,
                    data.actualCases.data
                )
                setData(processedData)
                const splitPeriods = processedData.map(
                    (item) => item.splitPoint
                )
    ...
    if (!data || !splitPeriods.length) return <div>Loading...</div>
